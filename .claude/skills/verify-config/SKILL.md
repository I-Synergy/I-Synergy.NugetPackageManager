...
1. **Read current documentation**
   - Read `CLAUDE.md`
   - Read `.claude/reference/critical-rules.md`
   - Read `.claude/patterns/cqrs-patterns.md`
...
4. **Check .claude/ folder structure**
   - Verify `.claude/settings.json` has `plansDirectory` pointing to local `.claude/plans`
   - Verify `.claude/progress/` and `.claude/plans/` folders exist
